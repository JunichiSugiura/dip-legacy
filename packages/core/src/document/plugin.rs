use crate::{
    command::UICommand,
    document::{info::DefaultEOL, Document},
    event::*,
};
...
fn insert(mut events: EventReader<DocumentInsert>, mut q: Query<(Entity, &mut Document)>) {
    for e in events.iter() {
        for (id, mut b) in q.iter_mut() {
            if id == e.entity {
                b.text_buffer.insert(e.offset, e.text);
            }
        }
    }
}
